...
- _ONCE_ accepted or YOLO, 
  - Derive one `{fix_key}`, record the default-branch base, mark every scoped finding `accepted` with that `Fix` and `Base-revision`, and create or compatibly resume `fix/{fix_key}`.
