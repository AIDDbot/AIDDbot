...
plus `e2e.plan.md` — ordered and actionable for `/codify`. On amend, always replan;
use checkpoints to keep, redo, or drop prior steps.

### Guardrails
- **Ground in the arch** — every step traces to the container's components and
  contracts.
- **Always replan after amend** — when the spec is `pending` from an amend or create,
  rewrite plans; never skip `/planify`.
- **Checkpoints control carry-forward** — on replan, mark each prior step
  keep / redo / drop before rewriting Implementation Steps.
...
### Inputs
- [ ] Required: a spec (`status: pending`), a short textual requirement, or a
      structural-refactor goal (no spec — behavior does not change).

### Glossary
- **Container** — a runnable unit in `system.arch.md` (`api`, `web`, `db`) — C4 L2.
- **Software container** — any container except `e2e`; planned from the solution
  overview.
- **e2e container** — transversal; planned via `e2e.plan.md` (one scenario per AC id);
  written by `/codify`, judged by `/verify`.
- **Checkpoint** — a prior plan step classified `keep` | `redo` | `drop` on
  amend/replan.
...
- _list_ the affected software containers and their solution outcomes
  (`e2e` excluded).
- _for-each_ non-`db` software container, _read_
  [its architecture]({Arch}/{container}.arch.md).
...
- _if_ not a structural refactor, _read_
  [e2e plan template](./assets/e2e.plan.template.md).
- _for-each_ software container:
  - _fill_ Checkpoints for every prior step (`keep` | `redo` | `drop`) when
    a prior plan exists; else note `none — first plan`.
  - _prepare_ Implementation Steps from keep/redo work plus new steps; all
    tasks `[ ]`.
...
- _if_ writing the e2e plan:
  - _derive_ it from the spec criteria and shared contracts, never from
    sibling code.
...
## Verification
- [ ] One plan per affected software container; `e2e.plan.md` present unless
      structural refactor.
- [ ] Each plan is grounded in its arch or `db.schema.md`, ordered, actionable.
- [ ] On replan, Checkpoints cover every prior step; Implementation Steps match
      keep/redo.
