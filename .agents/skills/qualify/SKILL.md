...
Report only — never edit code. Scope is the spec in flight — paths from that spec's plans plus its spec folder. On a shared change branch, do not judge the whole branch diff; stay spec-local. A gate passes only when you can say what you checked it against; one violation fails it. A container rule being written down does not mean it was applied. Findings correct the implementation, never the behavior.
