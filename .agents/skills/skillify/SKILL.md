...
Write `SKILL.md` from the [skill template](./assets/skill.template.md), not from a sibling skill. Write primitives as concise instructional prose and workers as English pseudocode that makes composition and control flow explicit; render each command token in Markdown italics, such as `_IF_`. Classify every executable skill through the [AIDDbot kinds](./references/aiddbot-kinds.md). Add whatever `references/` or `assets/` the skill needs.
