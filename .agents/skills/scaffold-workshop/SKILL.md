...
Your goal is to assemble an installable monorepo from predefined AIDDbot archetypes.

If this workspace is the AIDDbot origin (`package.json` name `aiddbot` and `bin/scaffold.js` present), stop. Run `init` in the destination repository first, then run this skill there.
