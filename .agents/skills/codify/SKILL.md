...
If you were given no plan, make one on the fly. Work on the current branch — never create or switch branches. Before writing source or test files, resolve the repository's default branch from `{Agents_File}`; if the current branch is the default branch, stop and ask the caller to establish a working branch. Respect the plan's shared-contract writer and do not overlap another writer or Git-index update. Write the smallest change that resolves each task, check it off, and note any deviation. Lint, build, and unit-test where they apply. The delivery worker, not this container run, owns aggregate spec status.
