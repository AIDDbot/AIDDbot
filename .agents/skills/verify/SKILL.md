...
**Single spec:** map functional scenarios from `e2e.plan.md`; write `{Product_Folder}/specs/{spec_key}/e2e.report.md` from the [defects report](./assets/e2e.report.template.md). Mark each active functional criterion from its result and set the spec to `verified` only on green. A technical spec has no functional E2E: run the existing suite as a regression net and leave its criteria for `qualify`.
