...
Increment the evaluation revision once per run, using the latest journaled revision when resuming. Journal every evaluation with its status and revision. When every applicable test passes, remove any existing `{Product_Folder}/specs/{spec_key}/verification.md` and write no report. For `amber` or `red`, write that file from `assets/verification.template.md` with only failing or blocked acceptance checks and their evidence; omit passing checks and general execution notes.
