...
Write `{Agents_Folder}/rules/{project}.rules.md` from the `project.rules.template.md`. Example: `./agents/rules/front.rules.md`
...
Write a database or API schema from its existing template only when the project exposes that shared contract. Use the `db.schema.template.md` from assets.
