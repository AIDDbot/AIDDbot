...
Unless in YOLO mode, present the spec and PRD edits for human approval and wait. Journal the creation and the approval. On approval, set `status` to `in-progress`, refresh `updated_at`, and set `last_process` to `approval`.
