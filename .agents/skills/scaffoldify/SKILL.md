---
name: scaffoldify
description: Build only the initial scaffold for a solution.
metadata:
...
Build only the initial scaffold for a solution. Do not implement features, business logic, product screens, or extra application code.
...
3. Run `node .agents/skills/scaffoldify/scripts/materialize.mjs --list` to inspect the archetype catalog before choosing technologies.
...
10. Reconcile main documents from each project to the solution root. Include README.md, LICENSE, and other project-specific documentation.
