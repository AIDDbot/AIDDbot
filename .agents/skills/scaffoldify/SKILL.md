...
Commit the scaffold changes on `chore/scaffold` as `chore(scaffold): add {containers}`. Include only changes belonging to this task. Once all selected scaffolds are created and committed, switch to the default branch and merge `chore/scaffold` into it. If scaffolding is incomplete, report the blocker without merging.
