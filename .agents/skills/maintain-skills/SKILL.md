...
Name assigned agents as **Architect**, **Builder**, or **Craftsman**. Refer to a skill by its backticked name, such as `define-spec`, and tell the assigned agent to execute it. Do not link directly to another skill's `SKILL.md` or tell an agent to read that file: native skill discovery and invocation must load the skill only when execution reaches it. Do not add `disable-model-invocation`; every capability remains available for model invocation.
