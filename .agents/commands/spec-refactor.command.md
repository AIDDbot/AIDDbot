...
Call `/specify` with `kind: refactor` to create it, then ask the human to check the result before
going any further.
...
flowchart TD
  ...
  S([start]):::start --> SPEC["/specify · kind: refactor"]:::nd --> CHK{"human validates"}:::hum
