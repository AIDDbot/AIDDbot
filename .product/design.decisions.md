...
`qualification.md` records findings from one spec, while `quality/review.md` records dated evidence from stricter system-wide checks. A finding becomes a TDR entry only after it is confirmed as debt that remains in shipped code. `shipify` asks `audit-quality` to promote surviving minor qualification findings or remove D IDs that a shipped repair proves resolved. Craftsman uses the same primitive to record system reviews, deduplicate debt, and maintain the TDR before selecting a coherent repair scope.
