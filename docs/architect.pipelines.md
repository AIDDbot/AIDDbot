...
`/explore` and `/extract` use the incremental artifact pattern (see each skill's `incremental-artifact.md`) — one file per run, `docs` commits. When complete, start features with `/specify`.
